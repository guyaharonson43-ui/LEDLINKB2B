  ...
  return (
    <>
          ...
          <div className="nav-desktop" style={{ display: 'flex', alignItems: 'center', gap: 4 }}>
            ...
            <div style={{ position: 'relative' }} onMouseEnter={() => setOpenDrop('catalog')} onMouseLeave={() => setOpenDrop(null)}>
              <a href="catalog.html" style={{ display: 'flex', alignItems: 'center', gap: 4, color: '#AAAAAA', textDecoration: 'none', fontSize: 15, fontWeight: 500, padding: '6px 14px', borderRadius: 6 }}>
                קטלוג מוצרים <span style={{ fontSize: 10, display: 'inline-block', transform: openDrop === 'catalog' ? 'rotate(180deg)' : 'none', transition: 'transform 0.2s' }}>▾</span>
              </a>
              {openDrop === 'catalog' && (
                <div style={dropBox}>
                  {CATALOG.map(item => <a key={item.url} href={item.url} className="nav-drop-item">{item.label}</a>)}
                </div>
              )}
            </div>
            ...
            <div style={{ position: 'relative' }} onMouseEnter={() => setOpenDrop('tools')} onMouseLeave={() => setOpenDrop(null)}>
              <a href="tools.html" style={{ display: 'flex', alignItems: 'center', gap: 4, color: '#AAAAAA', textDecoration: 'none', fontSize: 15, fontWeight: 500, padding: '6px 14px', borderRadius: 6 }}>
                כלי תכנון <span style={{ fontSize: 10, display: 'inline-block', transform: openDrop === 'tools' ? 'rotate(180deg)' : 'none', transition: 'transform 0.2s' }}>▾</span>
              </a>
              {openDrop === 'tools' && (
                <div style={dropBox}>
                  {TOOLS.map(item => <a key={item.url} href={item.url} className="nav-drop-item">{item.label}</a>)}
                </div>
              )}
            </div>

            <a href="guides.html" style={{ color: '#E8A020', textDecoration: 'none', fontSize: 15, fontWeight: 700, padding: '6px 14px', borderRadius: 6, background: 'rgba(232,160,32,0.1)' }}>מדריכים</a>
      ...
      <div className={menuOpen ? 'mobile-menu open' : 'mobile-menu'}>
        ...
        <a href="guides.html" className="mm-link active">מדריכים</a>
        <div className="mm-divider"></div>
